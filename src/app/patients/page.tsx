...
export default function PatientsPage({ searchParams }: { searchParams: { q?: string; status?: 'aktiv'|'inaktiv' }}) {
  const rows = filterPatients(searchParams.q, searchParams.status)

  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Patienten</h1>
        <Link href="/patients/new" className="inline-flex items-center gap-2 rounded-md bg-white/10 hover:bg-white/20 text-white px-3 py-2">
      ...
      <form className="mb-4 flex flex-wrap gap-2" method="get">
        ...
        <button className="px-3 py-2 rounded-md bg-red-600 text-white hover:bg-red-700">Filter anwenden</button>
        { (searchParams.q || searchParams.status) && (
          <Link href="/patients" className="px-3 py-2 rounded-md bg-white/10 hover:bg-white/20 text-white">Zurücksetzen</Link>
        )}
      </form>
      ...
      <div className="grid gap-3">
        {rows.map(p => (
          <Link key={p.id} href={`/patients/${p.id}`} className="group rounded-xl border bg-white hover:bg-gray-50 transition p-4 flex items-center gap-4">
