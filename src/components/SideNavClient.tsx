...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="px-4 pt-4 pb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
      {children}
    </div>
  );
}
  ...
  return (
    <aside className="w-64 bg-sky-100 h-screen flex flex-col rounded-r-3xl shadow-md border-r border-sky-200">
      ...
      <nav className="flex-1 py-2">
        ...
        <NavItem href="/" icon={<LayoutDashboard size={16} />}>
          Dashboard
        </NavItem>

        <SectionLabel>Bewohner</SectionLabel>
        <NavItem href="/patients" icon={<Users size={16} />}>
          Patienten
        </NavItem>
        <NavItem href="/relatives" icon={<Users size={16} />}>
          Angehörige
        </NavItem>
        <NavItem href="/medication" icon={<Pill size={16} />}>
          Medikamentenplan
        </NavItem>
        <NavItem href="/rooms" icon={<BedDouble size={16} />}>
          Zimmer
        </NavItem>

        <SectionLabel>Planung</SectionLabel>
        <NavItem href="/calendar" icon={<Calendar size={16} />}>
          Kalender
        </NavItem>
        <NavItem href="/roster" icon={<Briefcase size={16} />}>
          Dienstplan
        </NavItem>

        <SectionLabel>Kommunikation</SectionLabel>
        <NavItem href="/mail" icon={<Mail size={16} />}>
          E-Mail
        </NavItem>
        <NavItem href="/whatsapp" icon={<MessageCircle size={16} />}>
          WhatsApp
        </NavItem>
        ...
        <NavItem href="/billing" icon={<FileText size={16} />}>
          Abrechnung
        </NavItem>
        <NavItem href="/admin" icon={<Settings size={16} />}>
          Admin
        </NavItem>
      </nav>

      {/* Footer */}
      <div className="px-4 py-3 text-[11px] text-slate-600 border-t border-sky-200">
        v0.2 • Demo
      </div>
    </aside>
  );
